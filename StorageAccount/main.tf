...
resource "azurerm_key_vault" "mytfkv" {
  name                = "${lower(var.projectname)}-${lower(var.zone)}-${var.environmentName}-${var.containerName}-kv"
  location            = azurerm_resource_group.myrg.location
  resource_group_name = azurerm_resource_group.myrg.name
...
resource "azurerm_key_vault_secret" "kvsecret" {
  name         = "${lower(var.projectname)}-${lower(var.zone)}-${var.environmentName}-${var.containerName}-kvsecret"
