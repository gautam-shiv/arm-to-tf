...
resource "azurerm_sql_server" "sqlserver" {
    name                         = "${lower(var.projectName)}${lower(var.zone)}-o9-${lower(var.envName)}sql"
...
resource "azurerm_key_vault_secret" "kv5" {
    ...
    value        = "upper(var.envName!var.databaseNameadmin)"
    key_vault_id = azurerm_key_vault.keyvault.id
}

resource "azurerm_key_vault_secret" "kv6" {
    name         = "${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}kv--${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}sqlsqlReadOnlyLogin"
    value        = "readerO9"
    key_vault_id = azurerm_key_vault.keyvault.id
}

resource "azurerm_key_vault_secret" "kv7" {
    name         = "${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}kv--${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}sqlsqlReadOnlyLoginPassword"
    value        = "nifiabibev@readerpswd!o9"
    key_vault_id = azurerm_key_vault.keyvault.id
}

resource "azurerm_key_vault_secret" "kv8" {
    name         = "${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}kv--${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}sqlsqlReadWriteLogin"
    value        = "readerwriterO9"
    key_vault_id = azurerm_key_vault.keyvault.id
}

resource "azurerm_key_vault_secret" "kv9" {
    name         = "${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}kv--${lower(var.projectName)}${lower(var.zone)}-${lower(var.envName)}sqlsqlReadWriteLoginPassword"
    value        = "nifiabibev@readerwriterpswd!o9"
    key_vault_id = azurerm_key_vault.keyvault.id
}